...
export default function Footer() {
  ...
  return (
    <footer className="bg-background border-t border-black/5 py-24 px-6 relative overflow-hidden mesh-gradient">
      {/* Immersive Glows */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
          ...
          <div className="col-span-1">
            ...
            <p className="text-gray-500 text-base leading-relaxed mb-8 max-w-xs">
              {company.tagline}. <br />
              <span className="text-gray-400 text-sm">{company.subtagline.split('.')[0]}.</span>
            </p>
          ...
          <div>
            <h4 className="text-[10px] font-bold tracking-[0.3em] uppercase text-brand-navy mb-8">Expertise</h4>
            <ul className="flex flex-col gap-4">
              {services.map((s) => (
                <li key={s.id}>
                  <Link href={`/services/${s.slug}`} className="text-sm text-gray-500 hover:text-brand-primary transition-colors font-medium">
                    {s.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <h4 className="text-[10px] font-bold tracking-[0.3em] uppercase text-brand-navy mb-8">Platforms</h4>
            <ul className="flex flex-col gap-4">
              {products.map((p) => (
                <li key={p.id}>
                  <Link href={`/products/${p.slug}`} className="text-sm text-gray-500 hover:text-brand-primary transition-colors font-medium">
                    {p.name}
                  </Link>
                </li>
              ))}
          ...
          {/* Connection Hub */}
          <div>
            <h4 className="text-[10px] font-bold tracking-[0.3em] uppercase text-brand-navy mb-8">Contact</h4>
            <div className="flex flex-col gap-8">
               ...
               <div className="space-y-2">
                 <a href={`mailto:${company.email}`} className="text-sm text-gray-500 hover:text-brand-navy transition-colors flex items-center gap-2 font-medium">
                   <div className="w-1.5 h-1.5 rounded-full bg-brand-primary" />
                   {company.email}
                 </a>
                 <a href={`tel:${company.phone}`} className="text-sm text-gray-500 hover:text-brand-navy transition-colors flex items-center gap-2 font-medium">
        ...
        <div className="border-t border-black/5 pt-12 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest text-gray-400">
              ...
              <Link href="/contact" className="hover:text-brand-navy transition-colors">Accessibility</Link>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center gap-2 text-[9px] font-bold tracking-[0.3em] uppercase text-brand-primary bg-brand-primary/5 border border-brand-primary/10 px-5 py-2.5 rounded-full shadow-sm">
