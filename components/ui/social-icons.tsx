...
export function SocialIcons({ compact = false }: { compact?: boolean }) {
  ...
  const displaySocials = compact ? socials.slice(0, 4) : socials;
  ...
  return (
    <div className="relative flex items-center gap-1 px-2 py-1.5 rounded-2xl bg-[#111827] border border-[#1E293B]">
      ...
      {displaySocials.map((social, index) => (
        ...
        >
          <span
            className={`absolute inset-0 rounded-lg bg-[#2563EB]/10 transition-all duration-300 ease-out ${
          ...
          <span
            className={`relative z-10 transition-all duration-300 ease-out ${
              hoveredIndex === index ? "text-[#2563EB] scale-110" : "text-[#64748B]"
            }`}
          >
            {social.icon}
          </span>
          ...
          <span
            className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-[2px] rounded-full bg-[#2563EB] transition-all duration-300 ease-out ${
              hoveredIndex === index ? "w-3 opacity-100" : "w-0 opacity-0"
            }`}
          />

          <span
            className={`absolute -top-10 left-1/2 -translate-x-1/2 px-2.5 py-1 rounded-lg bg-[#2563EB] text-white text-[10px] font-bold tracking-widest uppercase whitespace-nowrap transition-all duration-300 ease-out ${
              hoveredIndex === index ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1 pointer-events-none"
            }`}
          >
            {social.name}
            <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 size-2 rotate-45 bg-[#2563EB]" />
          </span>
        </a>
      ))}
    </div>
